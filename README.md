...
module.log("example_metric", 123)  # Logs to wandb_wrapper as LoggingModule1_example_metric

module.log("example_metric", 234, skip_prefix=True)  # Logs to wandb_wrapper as example_metric
...
If no name is provided during initialization, `LoggingModule` automatically assigns unique names like `LoggingModule1`, `LoggingModule2`, etc. These names are used as prefixes for logged metrics, making it easy to identify the source of each log entry. But a name can also be given explicitly to each module.
...
LoggingModule.switch_all_logging(MyModel(), logging_active=True)
```

### Additional Features

- **Logging Prefixes**: Each `LoggingModule` instance prefixes metrics with its name unless otherwise specified, which is particularly useful for identifying metrics in nested structures.
...
- **Read-Only `loss_active` Property**: This property provides information on whether a given `LossModule` instance is currently set to compute losses.
...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        self.loss_module = CustomLossModule(logging_active=True)
